...
import { Resource, resources as seedData } from '../data';
...
const AdminPage = () => {
    ...
    const [formData, setFormData] = useState<Partial<Resource> & { logoUrl?: string }>({
        title: '',
        description: '',
        url: '',
        categoryId: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (editingId) {
            await editResource(editingId, {
                ...formData,
                tags,
                // If logoUrl is provided, use it as iconName, otherwise keep existing
                iconName: formData.logoUrl || formData.iconName
            });
            ...
            const newResource = {
                ...
                // If logoUrl is provided, use it as iconName, otherwise use existing iconName or default
                iconName: formData.logoUrl || formData.iconName || 'solar:link-circle-linear'
            };
    ...
    const handleEditClick = (resource: Resource) => {
        setFormData({
            ...
            // @ts-ignore
            logoUrl: resource.iconName?.startsWith('http') ? resource.iconName : ''
        });
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-2">
                        ...
                        <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-800">
                            <button
                                onClick={async () => {
                                    if (confirm('This will attempt to add all tools from data.ts to the database. Continue?')) {
                                        let addedCount = 0;
                                        for (const tool of seedData) {
                                            // Check if exists by URL to avoid dupes
                                            const exists = resources.some(r => r.url === tool.url);
                                            if (!exists) {
                                                await addResource(tool);
                                                addedCount++;
                                            }
                                        }
                                        alert(`Seeding complete! Added ${addedCount} new tools.`);
                                    }
                                }}
                                className="w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-colors text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
                            >
                                <IconifyIcon icon="solar:database-linear" width="20" />
                                Seed Database
                            </button>
                        </div>
                    </div>        </div>
                                ...
                                <form onSubmit={handleSubmit} className="mb-8 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-top-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="md:col-span-2">
                                            ...
                                            <input name="logoUrl" type="url" value={formData.logoUrl || ''} onChange={handleInputChange} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm" placeholder="https://example.com/logo.png" />
                                        </div>
                                        <div>
                                        ...
                                        {resources.map((resource) => (
                                            <tr key={resource.id} className="bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                ...
                                                <td className="hidden md:table-cell px-4 py-3 text-slate-500 dark:text-slate-400">
                                                    <span className={`px-2 py-0.5 rounded text-xs ${resource.type === 'Free' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
